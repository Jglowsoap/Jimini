...
from jimini_cli.loader import load_rules_from_file, load_rules_from_pack
...
def _print_sarif(decision: str, rule_ids: List[str]):
    sarif = {
      ...
      "runs": [{
        "tool": {"driver": {"name": "Jimini", "informationUri": "https://example.local/jimini"}},
        "results": [{
          "ruleId": rid,
          "level": "error" if decision == "block" else "warning" if decision == "flag" else "note",
          "message": {"text": f"Rule {rid} triggered. Decision: {decision}."}
...
def cmd_run_local(args):
    ...
    if args.rule_pack and not args.rules:
        pack_path = os.path.abspath(os.path.join(REPO_ROOT, "packs", args.rule_pack.lower(), f"{args.version}.yaml"))
        if not os.path.exists(pack_path):
            raise SystemExit(f"Pack not found: {pack_path}")
        env["RULES_PATH"] = pack_path
    else:
        env["RULES_PATH"] = os.path.abspath(args.rules)
    ...
    print(f"Launching Jimini on http://{host}:{port} with RULES_PATH={env['RULES_PATH']}"
...
def build_parser():
    ...
    p_lint.add_argument("--rule-pack", choices=["illinois","cjis","hipaa","pci","secrets"], help="Built-in rule pack")
    ...
    p_test.add_argument("--rule-pack", choices=["illinois","cjis","hipaa","pci","secrets"], help="Built-in rule pack")
    ...
    p_test.add_argument("--shadow", action="store_true", help="Simulate (never nonzero exit)")
    ...
    p_run.add_argument("--rule-pack", choices=["illinois","cjis","hipaa","pci","secrets"], help="Built-in rule pack")
    ...
    p_run.add_argument("--shadow", action="store_true", help="Simulate enforcement (no blocking)")
